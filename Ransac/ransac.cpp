...
void CRANSAC::convert_two_point_to_line()
{
	...
	lineEquation.m = (pt2.y-pt1.y)/double(pt2.x-pt1.x);
...
void CRANSAC::calculate_inlier()
{
	...
	for (int i = 0; i < point_vector_list.size(); i++)
	{
		if (i != pt1Idx && i != pt2Idx)
		{
			...
			// ** 지금부터 코드를 작성하세요. 이 줄은 지우시면 안 됩니다 **
			double distance = fabs(((-point_vector_list[i].x * lineEquation.m)-lineEquation.b+ point_vector_list[i].y)
				/ sqrt(lineEquation.m * lineEquation.m + 1 ) ); //여기
			// ** 여기까지 코드를 작성하세요. 이 줄은 지우시면 안 됩니다 **
			if (distance <= 5)
